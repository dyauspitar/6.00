...
"""
    ...
    while (r<30):
        ...
        p=Patient(viruses,1000)
        while (j<0):
            means_150.append(p.update())
            j+=1
        p.addPrescription('guttagonol')
        while (k<150):
            means_150.append(p.update())
            k+=1
        means_300[r]=means_150[-1]
        r+=1
    #pylab.figure()
...
"""
    ...
    p=Patient(viruses,1000)
    while (j<150):
        means_tot.append(p.update())
        means_rgu.append(p.getResistPop(['guttagonol']))
        means_rgr.append(p.getResistPop(['grimpex']))
        means_rbo.append(p.getResistPop(['guttagonol','grimpex']))
        j+=1
    p.addPrescription('guttagonol')
    while (k<150):
        means_tot.append(p.update())
    ...
    while (l<150):
        ...
        l+=1
    pylab.figure()
    p1=pylab.plot(means_tot, label='total')
    p2=pylab.plot(means_rgu, label='resist guttagonol')
    p3=pylab.plot(means_rgr, label='resist grimpex')
    p4=pylab.plot(means_rbo, label='resist both')
